...
export default function CreateEvent() {
    ...
    const handleCreate = async (e: React.FormEvent) => {
        ...
        if (!name || !date || !time || !price || !capacity || !locationName || !city || !country || !imageURL) {
            return toast.warning("⚠️ Lütfen tüm alanları doldurun!", { theme: "dark" });
        }

        // FİYAT KONTROLÜ
        if (Number(price) < 0) {
            return toast.warning("⚠️ Bilet fiyatı 0'dan küçük olamaz!", { theme: "dark" });
        }

        // KAPASİTE KONTROLÜ
        if (Number(capacity) <= 0) {
            return toast.warning("⚠️ Kapasite en az 1 kişi olmalıdır!", { theme: "dark" });
        }
        ...
        if (selectedDateTime <= now) {
            return toast.warning("⚠️ Geçmiş bir tarih veya saate etkinlik oluşturamazsınız!", { theme: "dark" });
        }
        ...
        try {
            ...
            if (error.code === "INSUFFICIENT_FUNDS" || error.message?.includes("insufficient funds")) {
                toast.error("⛔️ Yetersiz Bakiye! İşlem ücreti (Gas) için cüzdanınızda yeterli ETH yok.", { theme: "dark" });
                return;
            }

            // 3. Genel Hata
            const errorMessage = error.reason || error.message || "Bilinmeyen bir hata oluştu.";
            toast.error("Hata: " + errorMessage, { theme: "dark" });
        } finally {
            setLoading(false);
        }
    };
            ...
            <div style={{
                ...
                <div style={formSectionStyle}>
                    <div style={headerStyle}>
                        <h1 style={{ ...titleStyle, fontSize: isMobile ? '28px' : '36px' }}>Etkinlik Tasarla</h1>
                        <p style={{ ...subtitleStyle, fontSize: isMobile ? '14px' : '16px' }}>Topluluğunuzu bir araya getirecek eşsiz bir deneyim yaratın.</p>
                    </div>

                    <form onSubmit={handleCreate} style={{ display: "flex", flexDirection: "column", gap: isMobile ? "20px" : "25px" }}>
                        ...
                        <div style={{ ...cardStyle, padding: isMobile ? '20px' : '30px' }}>
                            ...
                            <div style={{ ...gridRowStyle, gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr' }}>
                                <div style={inputWrapperStyle}>
...
// --- STİLLER ---
const pageWrapperStyle: React.CSSProperties = { minHeight: '100vh', backgroundColor: BG_COLOR, color: 'white', fontFamily: '"Inter", sans-serif', borderRadius: '12px' };
const contentGridStyle: React.CSSProperties = { display: 'grid', maxWidth: '1100px', margin: '0 auto' };
...
const titleStyle: React.CSSProperties = { fontWeight: '900', margin: '0 0 10px 0', letterSpacing: '-1.5px', color: 'white' };
const subtitleStyle: React.CSSProperties = { color: TEXT_MUTED, margin: 0, fontWeight: '400', lineHeight: '1.5' };
const cardStyle: React.CSSProperties = { backgroundColor: CARD_BG, borderRadius: '16px', border: `1px solid ${BORDER_COLOR}`, boxShadow: '0 4px 15px rgba(0, 0, 0, 0.3)' };
...
const gridRowStyle: React.CSSProperties = { display: 'grid', gap: '20px' };
...
const stickyWrapperStyle: React.CSSProperties = { position: 'sticky' };
