...
export default function MyEvents() {
    ...
    const withdraw = async (eventAddress: string) => {
        try {
            ...
            toast.info("💸 Çekim işlemi blokzincire gönderildi... ⏳", { theme: "dark" });
            await tx.wait();
            toast.success("🤑 Tutar başarıyla cüzdana aktarıldı!", { theme: "dark" });
            window.location.reload();
        } catch (err: any) {
            console.error("Withdraw Error:", err);

            if (err.code === 4001 || err.message?.includes("user rejected") || err.reason?.includes("rejected")) {
                toast.warning("İşlem iptal edildi.", { theme: "dark" });
                return;
            }

            if (err.reason) {
                if (err.reason.includes("Etkinlik bitmeden")) {
                    toast.warning("⏳ Etkinlik süresi dolmadan hasılat çekilemez!", { theme: "dark" });
                } else if (err.reason.includes("iptal edilmis")) {
                    toast.error("🛑 İptal edilen etkinlikten para çekilemez!", { theme: "dark" });
                } else {
                    toast.error("Hata: " + err.reason, { theme: "dark" });
                }
            } else {
                toast.error("İşlem başarısız. Lütfen tekrar deneyin.", { theme: "dark" });
            }
        }
    };
    ...
    const handleConfirmCancel = async () => {
        ...
        try {
            ...
            toast.info("⏳ İptal işlemi blokzincire gönderildi...", { theme: "dark" });
            await tx.wait();
            toast.success("🛑 Etkinlik başarıyla iptal edildi.", { theme: "dark" });
            ...
            if (err.code === 4001 || err.message?.includes("user rejected")) {
                toast.warning("İptal işlemi vazgeçildi.", { theme: "dark" });
            } else if (err.reason) {
                if (err.reason.includes("Gecmis etkinlik")) {
                    toast.error("⌛️ Geçmiş veya süresi dolmuş etkinlikler iptal edilemez.", { theme: "dark" });
                } else if (err.reason.includes("Zaten iptal")) {
                    toast.warning("⚠️ Bu etkinlik zaten iptal edilmiş.", { theme: "dark" });
                } else {
                    toast.error("Hata: " + err.reason, { theme: "dark" });
                }
            } else {
                toast.error("İşlem sırasında bir hata oluştu.", { theme: "dark" });
            }
        } finally {
    ...
    const copyToClipboard = (text: string) => { navigator.clipboard.writeText(text); toast.success("Kopyalandı!", { theme: "dark", autoClose: 1000 }); };
    ...
    return (
        <div style={{ minHeight: '100vh', background: 'linear-gradient(180deg, #ffffff 0%, #f1f5f9 100%)', paddingBottom: '80px', fontFamily: "'Inter', sans-serif" }}>
            ...
            {/* HEADER */}
            <div style={{ padding: isMobile ? '30px 0 20px' : '60px 0 40px', backgroundColor: 'white', borderBottom: '1px solid #f1f5f9' }}>
                <div style={{ ...containerStyle, padding: isMobile ? '0 20px' : '0 25px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                        ...
                        <div>
                            <h1 style={{ margin: 0, fontSize: isMobile ? '24px' : '32px', color: '#0f172a', fontWeight: '900', letterSpacing: '-1px' }}>Organizatör Paneli</h1>
                            <p style={{ margin: '4px 0 0', color: '#64748b', fontSize: isMobile ? '13px' : '15px', fontWeight: '500' }}>Finansal durumunu ve etkinliklerini buradan yönet.</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div style={{ ...containerStyle, padding: isMobile ? '0 20px' : '0 25px' }}>
                ...
                <div style={{ marginTop: '50px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '25px' }}>
                        <h3 style={{ fontSize: isMobile ? '18px' : '20px', fontWeight: '800', color: '#0f172a', margin: 0 }}>Yönetim Merkezi <span style={{ fontSize: '14px', color: '#94a3b8', fontWeight: '500' }}>({myEvents.length} Etkinlik)</span></h3>
                        {myEvents.length > 0 && (
                    ...
                    {myEvents.length === 0 ? (
                        <div style={emptyStateStyle}>
                            <div style={{ fontSize: isMobile ? '50px' : '70px', marginBottom: '20px', opacity: 0.8 }}>🚀</div>
                            <h2 style={{ color: '#1e293b', marginBottom: '10px', fontSize: isMobile ? '20px' : '24px' }}>Yolculuk Başlasın!</h2>
                            <p style={{ color: '#64748b', maxWidth: '400px', margin: '0 auto 30px', fontSize: isMobile ? '14px' : '16px' }}>Henüz bir etkinlik oluşturmadın.</p>
                                    ...
                                    <div key={index} style={{ ...eventCardStyle, filter: evt.isCancelled ? 'grayscale(1)' : 'none', opacity: evt.isCancelled ? 0.8 : 1 }}>
                                        ...
                                        <div style={cardBodyStyle}>
                                            <div style={{ marginBottom: '20px' }}>
                                                <h3 style={{ ...eventTitleStyle, fontSize: isMobile ? '18px' : '20px' }}>{evt.name}</h3>
                                            ...
                                            <div style={{
                                                ...
                                                <span style={{ fontSize: isMobile ? '16px' : '18px', fontWeight: '900', color: hasBalance ? '#059669' : '#94a3b8', letterSpacing: '-0.5px' }}>{evt.balance} ETH</span>
                                            </div>
                                            ...
                                            <div style={{ display: 'flex', gap: '12px', marginTop: 'auto' }}>
                                                <button
                                                    ...
                                                    style={{
                                                        ...
                                                        fontSize: isMobile ? '12px' : '14px', fontWeight: '800', cursor: 'pointer',
...
const statsGridStyle: React.CSSProperties = { display: 'grid', gap: '25px', marginTop: '-30px' };
...
const eventTitleStyle: React.CSSProperties = { margin: '0 0 6px 0', fontWeight: '800', color: '#0f172a', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' };
